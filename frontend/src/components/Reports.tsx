  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-4 sm:p-6">
      ...
      <div className="mb-6 sm:mb-8">
        <h1 className="text-2xl sm:text-3xl font-bold text-slate-900">Relatórios</h1>
        <p className="text-slate-600 mt-1 sm:mt-2 text-sm sm:text-base">Análise detalhada de vendas e produtos</p>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="flex items-end">
            <button
              ...
              className="w-full px-4 py-2 h-11 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors"
            >
              Atualizar
            </button>
          </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-xs sm:text-sm min-w-[520px]">
                <thead className="bg-slate-100">
                  <tr>
                    <th className="px-3 sm:px-4 py-2 text-left">Cliente</th>
                    <th className="px-3 sm:px-4 py-2 text-right">Total</th>
                    <th className="px-3 sm:px-4 py-2 text-right">Pedidos</th>
                  </tr>
                </thead>
                <tbody>
                  {topCustomers.customers.map((customer: any, idx: number) => (
                    <tr key={idx} className="border-b hover:bg-slate-50">
                      <td className="px-3 sm:px-4 py-2 sm:py-3 font-medium">{customer.customer_name}</td>
                      <td className="px-3 sm:px-4 py-2 sm:py-3 text-right">
                        R$ {customer.total_spent.toLocaleString('pt-BR', { maximumFractionDigits: 0 })}
                      </td>
                      <td className="px-3 sm:px-4 py-2 sm:py-3 text-right">{customer.quote_count}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="bg-white rounded-lg shadow-md p-4 sm:p-6">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-xs sm:text-sm min-w-[520px]">
                <thead className="bg-slate-100">
                  <tr>
                    <th className="px-3 sm:px-4 py-2 text-left">Serviço</th>
                    <th className="px-3 sm:px-4 py-2 text-right">Qtd</th>
                    <th className="px-3 sm:px-4 py-2 text-right">Receita</th>
                  </tr>
                </thead>
                <tbody>
                  {topServices.services.map((service: any, idx: number) => (
                    <tr key={idx} className="border-b hover:bg-slate-50">
                      <td className="px-3 sm:px-4 py-2 sm:py-3 font-medium">{service.name}</td>
                      <td className="px-3 sm:px-4 py-2 sm:py-3 text-right">{service.quantity}</td>
                      <td className="px-3 sm:px-4 py-2 sm:py-3 text-right">
        ...
        <div className="bg-white rounded-lg shadow-md p-4 sm:p-6 mt-6">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs sm:text-sm min-w-[600px]">
              <thead className="bg-slate-100">
                <tr>
                  <th className="px-3 sm:px-4 py-2 text-left">Produto</th>
                  <th className="px-3 sm:px-4 py-2 text-right">Quantidade</th>
                  <th className="px-3 sm:px-4 py-2 text-right">Preço Unit.</th>
                  <th className="px-3 sm:px-4 py-2 text-right">Receita</th>
                </tr>
              </thead>
              <tbody>
                {topProducts.products.map((product: any, idx: number) => (
                  <tr key={idx} className="border-b hover:bg-slate-50">
                    <td className="px-3 sm:px-4 py-2 sm:py-3 font-medium">{product.name}</td>
                    <td className="px-3 sm:px-4 py-2 sm:py-3 text-right">{product.quantity}</td>
                    <td className="px-3 sm:px-4 py-2 sm:py-3 text-right">
