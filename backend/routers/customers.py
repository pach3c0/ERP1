...
# Função Auxiliar para Enviar Notificação Real-time
...
def list_customers(session: Session = Depends(get_session), current_user: User = Depends(get_current_user)):
    ...
    if role_slug == 'sales':
        statement = select(Customer).where(Customer.salesperson_id == current_user.id).order_by(Customer.id.desc())
    else:
...
def update_customer(
    ...
    role_slug = get_user_role_slug(current_user, session)
    if role_slug == 'sales' and customer.salesperson_id != current_user.id:
        raise HTTPException(status_code=403, detail="Este cliente não é seu.")
    ...
    if customer_data.status != customer.status:
        ...
        if not can_change: raise HTTPException(status_code=403, detail="Sem permissão de status.")
        status_changed = True

    data = customer_data.dict(exclude_unset=True)
    register_audit(session, current_user, customer, data, "customer", "UPDATE")
    ...
    if status_changed:
        ...
        if customer.status == 'ativo' and customer.salesperson_id and customer.salesperson_id != current_user.id:
             msg = f"Cliente {customer.name} APROVADO por {current_user.name}!"
...
def delete_customer(customer_id: int, session: Session = Depends(get_session), current_user: User = Depends(get_current_user)):
    if get_user_role_slug(current_user, session) not in ['admin', 'manager']:
        raise HTTPException(status_code=403, detail="Apenas Gerentes/Admins.")
    customer = session.get(Customer, customer_id)
    if not customer: raise HTTPException(status_code=404, detail="Não encontrado")
...
def get_customer_audit(customer_id: int, session: Session = Depends(get_session), current_user: User = Depends(get_current_user)):
    if get_user_role_slug(current_user, session) != 'admin':
        raise HTTPException(status_code=403, detail="Apenas Admin vê logs técnicos.")
    ...
    for log, user_name in results:
        logs.append(AuditLogRead(id=log.id, table_name=log.table_name, action=log.action, user_id=log.user_id, user_name=user_name, changes=log.changes, created_at=log.created_at))
    return logs
...
def create_customer_note(
    ...
    # 2. Notificação por Menção ou Alvo
    if note_data.target_user_id and note_data.target_user_id != current_user.id:
        ...
        if target_user:
            msg = "uma Tarefa" if note_data.type == 'task' else "uma Mensagem"
            content = f"{current_user.name} enviou {msg} em {customer.name}"
    ...
    # 3. Notificação Automática para o Dono (Sem menção)
...
def task_action(
    ...
    if not note: raise HTTPException(status_code=404, detail="Não encontrado")
    
    if action == "start":
        note.task_status = "in_progress"; note.started_at = datetime.utcnow()
        if note.created_by_id != current_user.id:
             msg = f"{current_user.name} iniciou tarefa em cliente #{note.customer_id}"
        ...
        note.task_status = "done"; note.completed_at = datetime.utcnow()
        if note.created_by_id != current_user.id:
             msg = f"{current_user.name} CONCLUIU tarefa!"
             session.add(Notification(user_id=note.created_by_id, content=msg, link=f"/customers/{note.customer_id}", related_note_id=note.id))
             background_tasks.add_task(notify_user_realtime, note.created_by_id, msg, f"/customers/{note.customer_id}")
